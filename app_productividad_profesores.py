...
# ==================================
# Aplicación Streamlit "Productividad de Profesionales"
# - Catálogos editables (Programas, Convenios, Instituciones, Profesionales, Pacientes)
# - Registrar atenciones con:
#   * Autorrelleno de paciente por documento
#   * Check de "Atención registrada en Panacea" (atención)
#   * Check de "Paciente creado en Panacea" (paciente)
#   * Check de "Paciente priorizado" con motivos visibles SOLO si está activo
# - Carga masiva (Excel/CSV) de atenciones, instituciones, profesionales, pacientes
# - Dashboard + Reportes (incluye viáticos, agenda, papelería)
# - Planificador, Viáticos, Papelería
# - Botón de respaldo de la base (admin)
...
# Motivos de priorización (aparecen SOLO si 'priorizado' = True)
MOTIVOS_PRIORIZACION = [
    "SG -SST FOMAG",
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    synonyms = {
        # generales
        "num_documento": "documento", "numero_documento": "documento",
        "nro_documento": "documento", "identificacion": "documento",
        "identificación": "documento", "cedula": "documento", "c_c": "documento",
        "nombre_completo": "nombre",
        "correo": "email", "correo_electronico": "email",
        "telefono_contacto": "telefono", "teléfono": "telefono",
        "dirección": "direccion",
        "fecha_de_nacimiento": "fecha_nacimiento",
        "zona_geografica": "zona",
        "actividad_plantilla": "actividad",
        "duracion": "duracion_minutos", "duracion_min": "duracion_minutos",
        "atendio": "atendido", "asistio": "atendido",
        ...
        # priorización
        "priorizado_paciente": "priorizado",
        "es_priorizado": "priorizado",
        "motivo_priorizado": "motivo_priorizacion",
    }
...
SQLITE_DDL = {
    ...
    CREATE TABLE IF NOT EXISTS registros(
        ...
        priorizado INTEGER,                   -- NUEVO
        motivo_priorizacion TEXT,             -- NUEVO
...
def ensure_sqlite_schema():
    with SQLITE_CONN:
        ...
        # Migraciones suaves
        def table_cols(tname: str) -> set:
            cur = SQLITE_CONN.execute(f"PRAGMA table_info({tname});")
            return {r["name"] for r in cur.fetchall()}

        # Activar 'activo' en catálogos si no existe
        for t in ["programas", "convenios", "instituciones", "profesores", "pacientes"]:
            have = table_cols(t)
            if "activo" not in have:
                SQLITE_CONN.execute(f"ALTER TABLE {t} ADD COLUMN activo INTEGER DEFAULT 1;")

        # Campos en registros
        rcols = table_cols("registros")
        adds = {
            "numero_paciente": "TEXT",
            "nombre_paciente": "TEXT",
            "actividad": "TEXT",
        ...
        for c, typ in adds.items():
            if c not in rcols:
                SQLITE_CONN.execute(f"ALTER TABLE registros ADD COLUMN {c} {typ};")

        # Columnas faltantes en viaticos/agenda/papeleria
        for t in ["viaticos", "agenda", "papeleria"]:
            have = table_cols(t)
            for need, typ in [
                ("programa_id", "INTEGER"),
                ("convenio_id", "INTEGER"),
                ...
                if need not in have:
                    SQLITE_CONN.execute(f"ALTER TABLE {t} ADD COLUMN {need} {typ};")
...
class DataAccess:
    ...
    def institucion_id_by_name_geo(self, nombre: str, municipio: Optional[str], departamento: Optional[str]) -> Optional[int]:
        if not nombre: return None
        ...
        r = self.db.execute(
            "SELECT id FROM instituciones WHERE nombre=? AND activo=1 ORDER BY id ASC", (nombre,)
        ).fetchone()
        return int(r["id"]) if r else None

    def profesor_id_by_name(self, nombre: str, programa_id: Optional[int], convenio_id: Optional[int]) -> Optional[int]:
        if not nombre: return None
        r = self.db.execute("SELECT id FROM profesores WHERE nombre=? AND activo=1 ORDER BY id ASC", (nombre,)).fetchone()
        return int(r["id"]) if r else None
    ...
    def update_programa(self, pid: int, nombre: str):
        with self.db:
            self.db.execute("UPDATE programas SET nombre=? WHERE id=?", (nombre.strip(), pid))

    def set_activo_programa(self, pid: int, activo: bool):
        with self.db:
            self.db.execute("UPDATE programas SET activo=? WHERE id=?", (1 if activo else 0, pid))
    ...
    def update_convenio(self, cid: int, nombre: str, programa_id: int):
        with self.db:
            self.db.execute("UPDATE convenios SET nombre=?, programa_id=? WHERE id=?", (nombre.strip(), programa_id, cid))

    def set_activo_convenio(self, cid: int, activo: bool):
        with self.db:
            self.db.execute("UPDATE convenios SET activo=? WHERE id=?", (1 if activo else 0, cid))
    ...
    def update_institucion(self, iid: int, nombre: str, localidad: Optional[str], municipio: Optional[str], departamento: Optional[str]):
        with self.db:
            self.db.execute(
                "UPDATE instituciones SET nombre=?, localidad=?, municipio=?, departamento=? WHERE id=?",
                (nombre.strip(), (localidad or None), (municipio or None), (departamento or None), iid),
            )

    def set_activo_institucion(self, iid: int, activo: bool):
        with self.db:
            self.db.execute("UPDATE instituciones SET activo=? WHERE id=?", (1 if activo else 0, iid))
    ...
    def update_profesor(self, fid: int, nombre: str, documento: Optional[str], email: Optional[str], programa_id: Optional[int], convenio_id: Optional[int], zona: Optional[str]):
        with self.db:
            self.db.execute(
                "UPDATE profesores SET nombre=?, documento=?, email=?, programa_id=?, convenio_id=?, zona=? WHERE id=?",
                (nombre.strip(), documento or None, email or None, programa_id, convenio_id, zona, fid),
            )

    def set_activo_profesor(self, fid: int, activo: bool):
        with self.db:
            self.db.execute("UPDATE profesores SET activo=? WHERE id=?", (1 if activo else 0, fid))
    ...
    def insert_registro(
        ...
        actividad, atendido, registrado_panacea, paciente_creado_panacea, priorizado, motivo_priorizacion,
        ...
        row = {
            ...
            "priorizado": 1 if priorizado else 0,
            "motivo_priorizacion": (motivo_priorizacion or None),
    ...
    def insert_agenda_event(self, fecha: date, hi: Optional[dtime], hf: Optional[dtime], titulo: str, descripcion,
                            programa_id, convenio_id, institucion_id, profesor_id, creado_por) -> None:
        row = {
            "fecha": fecha.strftime("%Y-%m-%d"),
    ...
    # Papelería
    def insert_papeleria(self, fecha: date, programa_id, convenio_id, institucion_id, profesor_id,
    ...
    # Utilidades admin: merge (fusionar) sin perder atenciones
    def merge_programas(self, src_id: int, dst_id: int, move_convenios: bool):
        if src_id == dst_id: raise ValueError("El programa origen y destino deben ser diferentes.")
        with self.db:
            for t in ["registros","viaticos","agenda","papeleria"]:
                self.db.execute(f"UPDATE {t} SET programa_id=? WHERE programa_id=?", (dst_id, src_id))
            if move_convenios:
                self.db.execute("UPDATE convenios SET programa_id=? WHERE programa_id=?", (dst_id, src_id))
            self.db.execute("UPDATE programas SET activo=0 WHERE id=?", (src_id,))

    def merge_convenios(self, src_id: int, dst_id: int):
        if src_id == dst_id: raise ValueError("El convenio origen y destino deben ser diferentes.")
        with self.db:
            for t in ["registros","viaticos","agenda","papeleria"]:
                self.db.execute(f"UPDATE {t} SET convenio_id=? WHERE convenio_id=?", (dst_id, src_id))
            self.db.execute("UPDATE convenios SET activo=0 WHERE id=?", (src_id,))
...
def sidebar_filters():
    ...
    # Respaldo de DB (solo admin)
    if st.session_state.role == "admin":
        try:
            with open(DB_SQLITE_PATH, "rb") as f:
                st.sidebar.download_button("⬇️ Respaldo base (.db)", data=f.read(), file_name=f"backup_{datetime.now().strftime('%Y%m%d_%H%M')}.db", key="backup_db")
        except Exception:
            pass
...
def plantilla_atenciones_df() -> pd.DataFrame:
    cols = [
        ...
        "paciente_creado_panacea",   # paciente creado
        "priorizado",                # NUEVO
        "motivo_priorizacion",       # NUEVO
...
def procesar_atenciones_masivo(df: pd.DataFrame, auth_user: str) -> Tuple[int, List[str]]:
    ...
    Opcionales: departamento, municipio, localidad, atendido, registrado_panacea, paciente_creado_panacea,
                priorizado, motivo_priorizacion, tipo_contacto, duracion_minutos, observaciones,
                sexo, fecha_nacimiento, telefono, email, direccion, zona
    """
    ...
    for idx, r in df.iterrows():
        try:
            ...
            # priorización
            prior = bool(str2bool(r.get("priorizado"))) if "priorizado" in df.columns else False
            motivo = str(r.get("motivo_priorizacion")).strip() if prior and pd.notna(r.get("motivo_priorizacion")) else None
            if prior and motivo and motivo not in MOTIVOS_PRIORIZACION:
                # Aceptar motivo libre pero normalizar capitalización
                motivo = motivo
            ...
            DATA.insert_registro(
                fecha=fecha,
                programa_id=pid, convenio_id=cid, institucion_id=iid, profesor_id=fid,
                paciente_id=pac_id,
                ...
                paciente_creado_panacea=paciente_creado_panacea,
                priorizado=prior, motivo_priorizacion=motivo,
...
def ui_cargar_datos(auth_user: Optional[str]):
    ...
    defaults = {
        ...
        K("priorizado"): False, K("motivo_prior"): MOTIVOS_PRIORIZACION[0],
    }
    ...
    # Priorizado + motivos condicionales
    c11, c12 = st.columns([1, 1])
    c11.checkbox("Paciente priorizado", key=K("priorizado"))
    if st.session_state[K("priorizado")]:
        c12.selectbox("Motivo de priorización", options=MOTIVOS_PRIORIZACION, key=K("motivo_prior"))
    else:
        st.session_state[K("motivo_prior")] = None
        c12.write("—")

    c13, c14 = st.columns([1, 1])
    c13.selectbox("Tipo de contacto", options=["(No especifica)"] + TIPOS_CONTACTO, key=K("tipo_contacto"))
    c14.number_input("Duración (minutos, opcional)", min_value=0, max_value=480, step=5, key=K("duracion_minutos"))
    ...
    if clicked:
        ...
        if faltantes:
            ...
            try:
                ...
                motivo_val = st.session_state.get(K("motivo_prior")) if st.session_state.get(K("priorizado")) else None
                ...
                DATA.insert_registro(
                    ...
                    profesor_id=int(fid), paciente_id=pac_id,
                    localidad=localidad_val, municipio=municipio_val, departamento=departamento_val,
                    ...
                    priorizado=bool(st.session_state[K("priorizado")]),
                    motivo_priorizacion=motivo_val,
    ...
    with st.expander("📥 Carga masiva de atenciones (Excel/CSV)", expanded=False):
        ...
        def to_excel_bytes(sheets: Dict[str, pd.DataFrame]) -> bytes:
            ...
            try:
                ...
                # Fallback simple a CSV único (si no hay openpyxl)
                csv = list(sheets.values())[0].to_csv(index=False).encode("utf-8")
                return csv
            return out.getvalue()
        ...
        st.caption("**Obligatorias**: fecha, programa, convenio, institucion, profesional, documento, nombre, actividad. "
                   ...
                   "paciente_creado_panacea (paciente), **priorizado**, **motivo_priorizacion**, tipo_contacto, duracion_minutos, observaciones, sexo, fecha_nacimiento, "
...
def ui_registros():
    ...
    # Mapear flags
    for col in ["atendido","registrado_panacea","paciente_creado_panacea","priorizado"]:
    ...
    show = [
        ...
        "atendido","priorizado","motivo_priorizacion",
        "paciente_creado_panacea","registrado_panacea",
...
def ui_dashboard():
    ...
    total_pan = int(df.get("registrado_panacea", pd.Series()).replace({"Sí":1,"No":0}).fillna(0).sum()) if "registrado_panacea" in df.columns else 0
    total_pac_creados = int(df.get("paciente_creado_panacea", pd.Series()).replace({"Sí":1,"No":0}).fillna(0).sum()) if "paciente_creado_panacea" in df.columns else 0
    ...
    if "registrado_panacea" in df.columns:
        # Convertir "Sí"/"No" a 1/0 si fuese necesario
        tmp = df.copy()
        if tmp["registrado_panacea"].dtype == object:
            tmp["registrado_panacea"] = tmp["registrado_panacea"].map({"Sí":1,"No":0}).fillna(0)
        pan = (
            tmp.groupby("profesor", dropna=True)
        ...
        fig2b = px.bar(
            ...
            x="profesor", y=["pacientes_atendidos", "cargadas_panacea"],
...
def ui_reportes():
    ...
    st.markdown("---")
    st.markdown("### Viáticos (reporte rápido)")
    dv = DATA.list_viaticos(st.session_state.filters)
    if dv.empty:
        st.info("Sin viáticos para el filtro.")
    else:
        st.dataframe(dv, use_container_width=True, hide_index=True)
        st.metric("Total viáticos (filtro)", f"${dv['valor'].fillna(0).sum():,.0f}".replace(",", "."))

    st.markdown("### Agenda (reporte rápido)")
    da = DATA.list_agenda(st.session_state.filters)
    if da.empty:
        st.info("Sin agenda para el filtro.")
    else:
        st.dataframe(da, use_container_width=True, hide_index=True)

    st.markdown("### Papelería (reporte rápido)")
    dp = DATA.list_papeleria(st.session_state.filters)
    if dp.empty:
        st.info("Sin solicitudes de papelería para el filtro.")
    else:
...
def ui_viaticos(auth_user: Optional[str]):
    ...
    if st.button("Guardar viático", type="primary", use_container_width=True, key="via_guardar"):
        try:
            DATA.insert_viatico(
                fecha=fecha, programa_id=pid, convenio_id=cid, institucion_id=iid, profesor_id=fid,
...
# ---------------- UI: CONFIGURACION (todo en una sola) ----------------
def ui_configuracion():
    st.subheader("Configuración (catálogos y edición)")
    ...
    with tabs[0]:
        ...
        dfp = DATA.list_programas()
        if dfp.empty:
            st.info("Sin programas.")
        else:
            st.dataframe(dfp, use_container_width=True, hide_index=True)
            e1, e2, e3 = st.columns([1, 2, 1])
            pid = e1.number_input("ID", min_value=1, step=1, key="cfg_prog_id")
            new_name = e2.text_input("Nuevo nombre", key="cfg_prog_newname")
            if e3.button("Actualizar", key="cfg_prog_upd"):
                try:
                    DATA.update_programa(int(pid), new_name or "")
                    success_toast("Programa actualizado."); st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo actualizar: {e}")
            d1, d2 = st.columns([1, 1])
            if d1.button("Desactivar", key="cfg_prog_off"):
                try:
                    DATA.set_activo_programa(int(pid), False); success_toast("Desactivado."); st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo desactivar: {e}")
            if d2.button("Activar", key="cfg_prog_on"):
                try:
                    DATA.set_activo_programa(int(pid), True); success_toast("Activado."); st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo activar: {e}")

            st.markdown("#### Fusionar programas")
            m1, m2, m3 = st.columns([1, 1, 2])
            src = m1.number_input("Programa origen (ID)", min_value=1, step=1, key="cfg_prog_merge_src")
            dst = m2.number_input("Programa destino (ID)", min_value=1, step=1, key="cfg_prog_merge_dst")
            mv = m3.checkbox("Mover convenios del origen al destino", value=True, key="cfg_prog_merge_mv")
            if st.button("Fusionar (mantiene registros)", type="primary", key="cfg_prog_do_merge"):
                try:
                    DATA.merge_programas(int(src), int(dst), bool(mv)); success_toast("Programas fusionados."); st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo fusionar: {e}")
    ...
    with tabs[1]:
        ...
        st.markdown("#### Editar / desactivar / fusionar")
        dcv = DATA.list_convenios()
        if dcv.empty:
            st.info("Sin convenios.")
        else:
            st.dataframe(dcv, use_container_width=True, hide_index=True)
            e1, e2, e3 = st.columns([1, 2, 2])
            cid = e1.number_input("ID", min_value=1, step=1, key="cfg_conv_id")
            new_name = e2.text_input("Nuevo nombre", key="cfg_conv_newname")
            new_prog = e3.selectbox("Nuevo programa", options=list(prog_map.keys()) if prog_map else [], key="cfg_conv_newprog")
            if st.button("Actualizar convenio", key="cfg_conv_upd"):
                try:
                    DATA.update_convenio(int(cid), new_name or "", prog_map.get(new_prog))
                    success_toast("Convenio actualizado."); st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo actualizar: {e}")
            d1, d2 = st.columns([1, 1])
            if d1.button("Desactivar", key="cfg_conv_off"):
                try:
                    DATA.set_activo_convenio(int(cid), False); success_toast("Desactivado."); st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo desactivar: {e}")
            if d2.button("Activar", key="cfg_conv_on"):
                try:
                    DATA.set_activo_convenio(int(cid), True); success_toast("Activado."); st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo activar: {e}")

            st.markdown("##### Fusionar convenios")
            m1, m2 = st.columns([1, 1])
            src = m1.number_input("Convenio origen (ID)", min_value=1, step=1, key="cfg_conv_merge_src")
            dst = m2.number_input("Convenio destino (ID)", min_value=1, step=1, key="cfg_conv_merge_dst")
            if st.button("Fusionar (mantiene registros)", type="primary", key="cfg_conv_do_merge"):
                try:
                    DATA.merge_convenios(int(src), int(dst)); success_toast("Convenios fusionados."); st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo fusionar: {e}")
    ...
    with tabs[2]:
        ...
        file_inst = st.file_uploader("Archivo (Excel o CSV)", type=["xlsx", "xls", "csv"], key="cfg_up_instituciones")
        ...
        st.markdown("---")
        st.markdown("#### Editar / activar / desactivar institución")
        ed1, ed2 = st.columns([1,3])
        iid = ed1.number_input("ID institución", min_value=1, step=1, key="cfg_inst_id")
        loaded = ed2.button("Cargar institución", key="cfg_inst_load")
        if loaded:
            row = SQLITE_CONN.execute("SELECT * FROM instituciones WHERE id=?", (int(iid),)).fetchone()
            if not row:
                warn_toast("No existe esa institución.")
            else:
                r = dict(row)
                f1, f2, f3, f4 = st.columns([2,1.2,1.2,1.2])
                nn = f1.text_input("Nombre", value=r["nombre"] or "", key="cfg_inst_edit_nom")
                nl = f2.text_input("Localidad", value=r["localidad"] or "", key="cfg_inst_edit_loc")
                nm = f3.text_input("Municipio", value=r["municipio"] or "", key="cfg_inst_edit_mun")
                nd = f4.text_input("Departamento", value=r["departamento"] or "", key="cfg_inst_edit_dep")
                b1, b2 = st.columns([1,1])
                if b1.button("Guardar cambios", key="cfg_inst_edit_save"):
                    try:
                        DATA.update_institucion(int(iid), nn, nl or None, nm or None, nd or None)
                        success_toast("Institución actualizada."); st.rerun()
                    except Exception as e:
                        error_toast(f"No se pudo actualizar: {e}")
                if b2.button("Desactivar", key="cfg_inst_edit_off"):
                    try:
                        DATA.set_activo_institucion(int(iid), False); success_toast("Desactivada."); st.rerun()
                    except Exception as e:
                        error_toast(f"No se pudo desactivar: {e}")
    ...
    with tabs[3]:
        ...
        st.markdown("---")
        st.markdown("#### Editar / activar / desactivar profesional")
        ed1, ed2 = st.columns([1,3])
        fid = ed1.number_input("ID profesional", min_value=1, step=1, key="cfg_prof_id")
        loaded = ed2.button("Cargar profesional", key="cfg_prof_load")
        if loaded:
            row = SQLITE_CONN.execute("SELECT * FROM profesores WHERE id=?", (int(fid),)).fetchone()
            if not row:
                warn_toast("No existe ese profesional.")
            else:
                r = dict(row)
                pf1, pf2, pf3 = st.columns([2,1,1])
                nn = pf1.text_input("Nombre", value=r["nombre"] or "", key="cfg_prof_edit_nom")
                nd = pf2.text_input("Documento", value=r["documento"] or "", key="cfg_prof_edit_doc")
                ne = pf3.text_input("Email", value=r["email"] or "", key="cfg_prof_edit_email")
                prog_sel = st.selectbox("Programa", options=["(Sin programa)"] + list(prog_map.keys()), index=(["(Sin programa)"]+list(prog_map.keys())).index(next((k for k,v in prog_map.items() if v==r["programa_id"]), "(Sin programa)")) if prog_map else 0, key="cfg_prof_edit_prog")
                conv_sel = st.selectbox("Convenio", options=["(Sin convenio)"] + list(conv_map.keys()), index=(["(Sin convenio)"]+list(conv_map.keys())).index(next((k for k,v in conv_map.items() if v==r["convenio_id"]), "(Sin convenio)")) if conv_map else 0, key="cfg_prof_edit_conv")
                zopts = ["(No especifica)", "Urbana", "Rural"]
                nz = st.selectbox("Zona", options=zopts, index=zopts.index(r["zona"]) if r["zona"] in zopts else 0, key="cfg_prof_edit_zona")
                b1, b2 = st.columns([1,1])
                if b1.button("Guardar cambios", key="cfg_prof_edit_save"):
                    try:
                        DATA.update_profesor(int(fid), nn, nd or None, ne or None, prog_map.get(prog_sel), conv_map.get(conv_sel), (None if nz=="(No especifica)" else nz))
                        success_toast("Profesional actualizado."); st.rerun()
                    except Exception as e:
                        error_toast(f"No se pudo actualizar: {e}")
                if b2.button("Desactivar", key="cfg_prof_edit_off"):
                    try:
                        DATA.set_activo_profesor(int(fid), False); success_toast("Desactivado."); st.rerun()
                    except Exception as e:
                        error_toast(f"No se pudo desactivar: {e}")
    ...
    with tabs[4]:
        ...
        cfg_nac = c3.text_input("Fecha nacimiento (AAAA-MM-DD, opcional)", key="cfg_pac_fecha_nac")
        sexo_opts = ["(No especifica)", "F", "M", "Otro"]
