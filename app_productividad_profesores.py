...
# ---------------- ADVANCED SQL HELPERS (non-destructive) ----------------
def _get_row(table: str, rid: int):
    cur = SQLITE_CONN.execute(f"SELECT * FROM {table} WHERE id=?", (rid,))
    r = cur.fetchone()
    return dict(r) if r else None

def _update_row(table: str, rid: int, updates: dict):
    if not updates: 
        return
    sets = ",".join([f"{k}=?" for k in updates.keys()])
    with SQLITE_CONN:
        SQLITE_CONN.execute(f"UPDATE {table} SET {sets} WHERE id=?", (*updates.values(), rid))

def _set_activo(table: str, rid: int, activo: int):
    with SQLITE_CONN:
        SQLITE_CONN.execute(f"UPDATE {table} SET activo=? WHERE id=?", (1 if activo else 0, rid))

def merge_programas(src_id: int, dst_id: int, move_convenios: bool=False):
    if src_id == dst_id:
        raise ValueError("El programa origen y destino deben ser diferentes.")
    with SQLITE_CONN:
        # Reasignar referencias
        for t in ["registros","viaticos","agenda","papeleria"]:
            SQLITE_CONN.execute(f"UPDATE {t} SET programa_id=? WHERE programa_id=?", (dst_id, src_id))
        # Opcional: mover convenios al programa destino
        if move_convenios:
            SQLITE_CONN.execute("UPDATE convenios SET programa_id=? WHERE programa_id=?", (dst_id, src_id))
        # Desactivar el programa origen
        SQLITE_CONN.execute("UPDATE programas SET activo=0 WHERE id=?", (src_id,))

def merge_convenios(src_id: int, dst_id: int):
    if src_id == dst_id:
        raise ValueError("El convenio origen y destino deben ser diferentes.")
    with SQLITE_CONN:
        for t in ["registros","viaticos","agenda","papeleria"]:
            SQLITE_CONN.execute(f"UPDATE {t} SET convenio_id=? WHERE convenio_id=?", (dst_id, src_id))
        SQLITE_CONN.execute("UPDATE convenios SET activo=0 WHERE id=?", (src_id,))
...
# ---------------- UI: CONFIGURACIÓN AVANZADA (edición / fusión / activar-desactivar) ----------------
def ui_configuracion_avanzada():
    st.subheader("Configuración avanzada (edición, fusión y activación)")

    tabs = st.tabs(["Editar catálogos", "Fusionar", "Activar / Desactivar"])
    ...
    with tabs[0]:
        ...
        progs = DATA.list_programas()
        if progs.empty:
            st.info("No hay programas activos.")
        else:
            pmap = {f"[{int(r['id'])}] {r['nombre']}": int(r["id"]) for _, r in progs.iterrows()}
            psel = st.selectbox("Programa a editar", options=list(pmap.keys()), key="adv_prog_sel")
            pid = pmap.get(psel)
            pnew = st.text_input("Nuevo nombre", value=psel.split("] ",1)[1] if psel else "", key="adv_prog_new")
            c1, c2 = st.columns([1,1])
            if c1.button("Guardar cambios", key="adv_prog_save"):
                if not pnew.strip():
                    warn_toast("El nombre no puede estar vacío.")
                else:
                    _update_row("programas", pid, {"nombre": pnew.strip()})
                    success_toast("Programa actualizado.")
                    st.rerun()

        st.markdown("---")
        st.markdown("#### Convenios")
        conv = DATA.list_convenios()
        if conv.empty:
            st.info("No hay convenios activos.")
        else:
            cmap = {f"[{int(r['id'])}] {r['nombre']} (prog {int(r['programa_id'])})": int(r["id"]) for _, r in conv.iterrows()}
            csel = st.selectbox("Convenio a editar", options=list(cmap.keys()), key="adv_conv_sel")
            cid = cmap.get(csel)
            cnew = st.text_input("Nuevo nombre", value=csel.split("] ",1)[1].split(" (prog",1)[0] if csel else "", key="adv_conv_new")
            if st.button("Guardar cambios de convenio", key="adv_conv_save"):
                if not cnew.strip():
                    warn_toast("El nombre no puede estar vacío.")
                else:
                    _update_row("convenios", cid, {"nombre": cnew.strip()})
        ...
        inst = DATA.list_instituciones()
        if inst.empty:
            st.info("No hay instituciones activas.")
        else:
            imap = {f"[{int(r['id'])}] {r['nombre']}": int(r["id"]) for _, r in inst.iterrows()}
            isel = st.selectbox("Institución a editar", options=list(imap.keys()), key="adv_inst_sel")
            iid = imap.get(isel)
            row = _get_row("instituciones", iid)
            c1, c2, c3, c4 = st.columns([2,1,1,1])
            inew = c1.text_input("Nombre", value=row.get("nombre","") if row else "", key="adv_inst_nom")
            iloc = c2.text_input("Localidad", value=row.get("localidad","") if row else "", key="adv_inst_loc")
            imun = c3.text_input("Municipio", value=row.get("municipio","") if row else "", key="adv_inst_mun")
            idep = c4.text_input("Departamento", value=row.get("departamento","") if row else "", key="adv_inst_dep")
            if st.button("Guardar cambios de institución", key="adv_inst_save"):
                _update_row("instituciones", iid, {
                    "nombre": inew.strip() or None,
                    "localidad": iloc.strip() or None,
                    "municipio": imun.strip() or None,
                    "departamento": idep.strip() or None,
                })
        ...
        prof = DATA.list_Profesionales()
        if prof.empty:
            st.info("No hay profesionales activos.")
        else:
            fmap = {f"[{int(r['id'])}] {r['nombre']}": int(r["id"]) for _, r in prof.iterrows()}
            fsel = st.selectbox("Profesional a editar", options=list(fmap.keys()), key="adv_prof_sel")
            fid = fmap.get(fsel)
            r = _get_row("Profesionales", fid)
            # catálogos para reasignación
            progs2 = DATA.list_programas()
            prog_map2 = {r2["nombre"]: int(r2["id"]) for _, r2 in progs2.iterrows()} if not progs2.empty else {}
            conv2 = DATA.list_convenios()
            conv_map2 = {r2["nombre"]: int(r2["id"]) for _, r2 in conv2.iterrows()} if not conv2.empty else {}

            c1, c2, c3 = st.columns([2,1,1])
            fnom = c1.text_input("Nombre", value=r.get("nombre","") if r else "", key="adv_prof_nom")
            fdoc = c2.text_input("Documento", value=r.get("documento","") if r else "", key="adv_prof_doc")
            femail = c3.text_input("Email", value=r.get("email","") if r else "", key="adv_prof_email")
            c4, c5, c6 = st.columns([1,1,1])
            fprog = c4.selectbox("Programa", options=list(prog_map2.keys()), index=max(0, list(prog_map2.values()).index(r.get("programa_id")) if r and r.get("programa_id") in prog_map2.values() else 0), key="adv_prof_prog")
            fconv = c5.selectbox("Convenio", options=list(conv_map2.keys()), index=max(0, list(conv_map2.values()).index(r.get("convenio_id")) if r and r.get("convenio_id") in conv_map2.values() else 0), key="adv_prof_conv")
            fzona = c6.selectbox("Zona", options=["(No especifica)","Urbana","Rural"], index=0 if not r or not r.get("zona") else ["(No especifica)","Urbana","Rural"].index(r.get("zona") if r.get("zona") in ["Urbana","Rural"] else "(No especifica)"), key="adv_prof_zona")

            if st.button("Guardar cambios de profesional", key="adv_prof_save"):
                _update_row("Profesionales", fid, {
                    "nombre": fnom.strip() or None,
                    "documento": fdoc.strip() or None,
                    "email": femail.strip() or None,
                    "programa_id": prog_map2.get(fprog),
                    "convenio_id": conv_map2.get(fconv),
                    "zona": None if fzona == "(No especifica)" else fzona,
                })
        ...
        pac = DATA.list_pacientes()
        if pac.empty:
            st.info("No hay pacientes activos.")
        else:
            pmap2 = {f"[{int(r['id'])}] {r['nombre']} ({r['numero_documento']})": int(r["id"]) for _, r in pac.iterrows()}
            psel2 = st.selectbox("Paciente a editar", options=list(pmap2.keys()), key="adv_pac_sel")
            pid2 = pmap2.get(psel2)
            pr = _get_row("pacientes", pid2)
            c1, c2 = st.columns([2,1])
            pnom = c1.text_input("Nombre", value=pr.get("nombre","") if pr else "", key="adv_pac_nom")
            pdoc = c2.text_input("Documento", value=pr.get("numero_documento","") if pr else "", key="adv_pac_doc")
            c3, c4, c5 = st.columns([1,1,1])
            pnac = c3.text_input("Fecha nacimiento (AAAA-MM-DD)", value=pr.get("fecha_nacimiento","") if pr else "", key="adv_pac_nac")
            psexo = c4.selectbox("Sexo", ["(No especifica)","F","M","Otro"], index=0 if not pr or not pr.get("sexo") else ["(No especifica)","F","M","Otro"].index(pr.get("sexo") if pr.get("sexo") in ["F","M","Otro"] else "(No especifica)"), key="adv_pac_sexo")
            pzona = c5.selectbox("Zona", ["(No especifica)","Urbana","Rural"], index=0 if not pr or not pr.get("zona") else ["(No especifica)","Urbana","Rural"].index(pr.get("zona") if pr.get("zona") in ["Urbana","Rural"] else "(No especifica)"), key="adv_pac_zona")
            c6, c7 = st.columns([1,1])
            ptel = c6.text_input("Teléfono", value=pr.get("telefono","") if pr else "", key="adv_pac_tel")
            pmail = c7.text_input("Email", value=pr.get("email","") if pr else "", key="adv_pac_mail")
            pdir = st.text_input("Dirección", value=pr.get("direccion","") if pr else "", key="adv_pac_dir")
            c8, c9, c10 = st.columns([1,1,1])
            ploc = c8.text_input("Localidad", value=pr.get("localidad","") if pr else "", key="adv_pac_loc")
            pmun = c9.text_input("Municipio", value=pr.get("municipio","") if pr else "", key="adv_pac_mun")
            pdep = c10.text_input("Departamento", value=pr.get("departamento","") if pr else "", key="adv_pac_dep")

            if st.button("Guardar cambios de paciente", key="adv_pac_save"):
                _update_row("pacientes", pid2, {
                    "numero_documento": pdoc.strip(),
                    "nombre": pnom.strip(),
                    "fecha_nacimiento": pnac.strip() or None,
                    "sexo": None if psexo == "(No especifica)" else psexo,
                    "telefono": ptel.strip() or None,
                    "email": pmail.strip() or None,
                    "direccion": pdir.strip() or None,
                    "localidad": ploc.strip() or None,
                    "municipio": pmun.strip() or None,
                    "departamento": pdep.strip() or None,
                    "zona": None if pzona == "(No especifica)" else pzona,
                })
                success_toast("Paciente actualizado.")
                st.rerun()
    ...
    with tabs[1]:
        ...
        progs = DATA.list_programas()
        if progs.empty:
            st.info("No hay programas.")
        else:
            pmap = {f"[{int(r['id'])}] {r['nombre']}": int(r["id"]) for _, r in progs.iterrows()}
            col1, col2, col3 = st.columns([1,1,1])
            src = col1.selectbox("Programa origen", options=list(pmap.keys()), key="adv_prog_src")
            dst = col2.selectbox("Programa destino", options=list(pmap.keys()), key="adv_prog_dst")
            move_c = col3.checkbox("Mover convenios al destino", value=False, key="adv_prog_movec")
            if st.button("Fusionar programas", type="primary", key="adv_prog_merge"):
                try:
                    merge_programas(pmap[src], pmap[dst], move_c)
                    success_toast("Programas fusionados (origen desactivado).")
                    st.rerun()
                except Exception as e:
        ...
        conv = DATA.list_convenios()
        if conv.empty:
            st.info("No hay convenios.")
        else:
            cmap = {f"[{int(r['id'])}] {r['nombre']} (prog {int(r['programa_id'])})": int(r["id"]) for _, r in conv.iterrows()}
            c1, c2 = st.columns([1,1])
            src = c1.selectbox("Convenio origen", options=list(cmap.keys()), key="adv_conv_src")
            dst = c2.selectbox("Convenio destino", options=list(cmap.keys()), key="adv_conv_dst")
            if st.button("Fusionar convenios", type="primary", key="adv_conv_merge"):
                try:
                    merge_convenios(cmap[src], cmap[dst])
                    success_toast("Convenios fusionados (origen desactivado).")
                    st.rerun()
                except Exception as e:
                    error_toast(f"No se pudo fusionar: {e}")
    ...
    # --- Activar / Desactivar ---
    with tabs[2]:
        st.caption("Desactiva elementos de catálogos sin borrar registros históricos.")
        cols = st.columns(3)
        with cols[0]:
            st.markdown("**Programas**")
            progs = DATA.list_programas()
            allp = SQLITE_CONN.execute("SELECT * FROM programas ORDER BY nombre").fetchall()
            if allp:
                pmap = {f"[{int(r['id'])}] {r['nombre']} (activo {int(r['activo'])})": int(r["id"]) for r in allp}
                psel = st.selectbox("Programa", options=list(pmap.keys()), key="adv_act_prog")
                pid = pmap[psel]
                new_state = st.radio("Estado", ["Activo","Inactivo"], index=0 if "activo 1" in psel else 1, key="adv_act_prog_state", horizontal=True)
                if st.button("Aplicar", key="adv_act_prog_apply"):
                    _set_activo("programas", pid, 1 if new_state=="Activo" else 0)
                    success_toast("Estado actualizado.")
                    st.rerun()

        with cols[1]:
            st.markdown("**Convenios**")
            allc = SQLITE_CONN.execute("SELECT * FROM convenios ORDER BY nombre").fetchall()
            if allc:
                cmap = {f"[{int(r['id'])}] {r['nombre']} (prog {int(r['programa_id'])}) (activo {int(r['activo'])})": int(r["id"]) for r in allc}
                csel = st.selectbox("Convenio", options=list(cmap.keys()), key="adv_act_conv")
                cid = cmap[csel]
                new_state = st.radio("Estado", ["Activo","Inactivo"], index=0 if "activo 1" in csel else 1, key="adv_act_conv_state", horizontal=True)
                if st.button("Aplicar ", key="adv_act_conv_apply"):
                    _set_activo("convenios", cid, 1 if new_state=="Activo" else 0)
                    success_toast("Estado actualizado.")
                    st.rerun()

        with cols[2]:
            st.markdown("**Instituciones**")
            alli = SQLITE_CONN.execute("SELECT * FROM instituciones ORDER BY nombre").fetchall()
            if alli:
                imap = {f"[{int(r['id'])}] {r['nombre']} (activo {int(r['activo'])})": int(r["id"]) for r in alli}
                isel = st.selectbox("Institución", options=list(imap.keys()), key="adv_act_inst")
                iid = imap[isel]
                new_state = st.radio("Estado", ["Activo","Inactivo"], index=0 if "activo 1" in isel else 1, key="adv_act_inst_state", horizontal=True)
                if st.button("Aplicar  ", key="adv_act_inst_apply"):
                    _set_activo("instituciones", iid, 1 if new_state=="Activo" else 0)
                    success_toast("Estado actualizado.")
                    st.rerun()

        st.markdown("---")
        st.markdown("**Profesionales y Pacientes**")
        colA, colB = st.columns(2)
        with colA:
            allf = SQLITE_CONN.execute("SELECT * FROM Profesionales ORDER BY nombre").fetchall()
            if allf:
                fmap = {f"[{int(r['id'])}] {r['nombre']} (activo {int(r['activo'])})": int(r["id"]) for r in allf}
                fsel = st.selectbox("Profesional", options=list(fmap.keys()), key="adv_act_prof")
                fid = fmap[fsel]
                new_state = st.radio("Estado", ["Activo","Inactivo"], index=0 if "activo 1" in fsel else 1, key="adv_act_prof_state", horizontal=True)
                if st.button("Aplicar   ", key="adv_act_prof_apply"):
                    _set_activo("Profesionales", fid, 1 if new_state=="Activo" else 0)
                    success_toast("Estado actualizado.")
                    st.rerun()
        with colB:
            allp2 = SQLITE_CONN.execute("SELECT * FROM pacientes ORDER BY nombre").fetchall()
            if allp2:
                pmap = {f"[{int(r['id'])}] {r['nombre']} (activo {int(r['activo'])})": int(r["id"]) for r in allp2}
                psel = st.selectbox("Paciente", options=list(pmap.keys()), key="adv_act_pac")
                pid = pmap[psel]
                new_state = st.radio("Estado", ["Activo","Inactivo"], index=0 if "activo 1" in psel else 1, key="adv_act_pac_state", horizontal=True)
                if st.button("Aplicar    ", key="adv_act_pac_apply"):
                    _set_activo("pacientes", pid, 1 if new_state=="Activo" else 0)
                    success_toast("Estado actualizado.")
                    st.rerun()


# ---------------- UI: REPORTES EXTRA (viáticos, planificador, papelería) ----------------
def ui_reportes_extra():
    st.subheader("Reportes: Viáticos, Planificador y Papelería (descarga)")
    dfv = DATA.list_viaticos(st.session_state.filters)
    dfa = DATA.list_agenda(st.session_state.filters)
    dfp = DATA.list_papeleria(st.session_state.filters)

    st.markdown("### Viáticos")
    if dfv.empty:
        st.info("Sin viáticos en el rango/criterios.")
    else:
        st.dataframe(dfv, use_container_width=True, hide_index=True)
    st.markdown("### Planificador")
    if dfa.empty:
        st.info("Sin eventos en agenda en el rango/criterios.")
    else:
        st.dataframe(dfa, use_container_width=True, hide_index=True)
    st.markdown("### Papelería")
    if dfp.empty:
        st.info("Sin solicitudes de papelería en el rango/criterios.")
    else:
    ...
    st.download_button(
        ...
        data=xls,
        file_name=f"reportes_extra_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
        key="rep_extra_xlsx"
    )
    # CSV individuales
    if not dfv.empty:
        st.download_button("CSV Viáticos", data=dfv.to_csv(index=False).encode("utf-8"), file_name="viaticos.csv", mime="text/csv", key="rep_extra_csv_v")
    if not dfa.empty:
        st.download_button("CSV Agenda", data=dfa.to_csv(index=False).encode("utf-8"), file_name="agenda.csv", mime="text/csv", key="rep_extra_csv_a")
    if not dfp.empty:
        st.download_button("CSV Papelería", data=dfp.to_csv(index=False).encode("utf-8"), file_name="papeleria.csv", mime="text/csv", key="rep_extra_csv_p")
def main():
